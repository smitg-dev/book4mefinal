export class PlanetViewer {
    ...
    draw() {
        ...
        const bgGrad = this.ctx.createRadialGradient(width / 2, height / 2, 50, width / 2, height / 2, Math.max(width, height) / 1.2);
        ...
        const sunGrad = this.ctx.createRadialGradient(centerX, centerY, 5, centerX, centerY, 35);
        ...
        this.bodies.forEach((body) => {
            ...
            this.ctx.beginPath();
            this.ctx.ellipse(centerX, centerY, body.orbitDistance, body.orbitDistance * 0.45, 0, 0, Math.PI * 2);
            this.ctx.stroke();
            this.ctx.setLineDash([]);
            // Body Glow
            const auraRad = body.radius * (isSelected ? 2.5 : isHovered ? 2.0 : 1.5);
            const glowGrad = this.ctx.createRadialGradient(bodyX, bodyY, body.radius * 0.5, bodyX, bodyY, auraRad);
            ...
            const shadowGrad = this.ctx.createRadialGradient(bodyX - body.radius * 0.4, bodyY - body.radius * 0.4, body.radius * 0.1, bodyX + body.radius * 0.3, bodyY + body.radius * 0.3, body.radius * 1.2);
            ...
            this.ctx.fillText(`${body.featuredImgSymbol} ${body.category.toUpperCase()}`, bodyX, bodyY + body.radius + 28);
        });
    }
}
