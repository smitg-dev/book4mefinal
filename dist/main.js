...
class BookFromSpaceApp {
    ...
    init() {
        this.manager = new BookingWizardManager(CELESTIAL_BODIES, SERVICES, LAUNCH_VEHICLES, SPACEPORTS, ADDONS);
        ...
        document.querySelectorAll('.craft-btn').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                ...
                if (modelType === 'glb') {
                    ...
                    if (stageSketchfab)
                        stageSketchfab.style.display = 'none';
                }
                else if (modelType === 'sketchfab') {
                    if (stageSketchfab) {
                        stageSketchfab.setAttribute('src', `https://sketchfab.com/models/${src}/embed?autostart=1&preload=1&ui_controls=1&ui_infos=0&ui_inspector=0&ui_stop=0&ui_watermark=0&ui_theme=dark`);
                        stageSketchfab.style.display = 'block';
                    }
                    if (stageGlb)
                        stageGlb.style.display = 'none';
                }
                const specData = CRAFT_SPECS[name];
                if (specData) {
                    if (specBadge)
                        specBadge.textContent = specData.badge;
                    if (specTitle)
                        specTitle.textContent = specData.title;
                    if (specGrid) {
    ...
    renderWizardStep() {
        ...
        if (btnNext)
            btnNext.textContent = step === 4 ? 'Confirm & Launch Reservation 🚀' : 'Next Step ➔';
        if (totalCostSpan) {
        ...
        const activeData = this.manager.startBookingFlow(this.manager.activeFormData?.serviceId);
        // Maintain form selections across steps
        const currentForm = this.manager.activeFormData;
        switch (step) {
            case 1:
                if (stepTitle)
                    stepTitle.textContent = 'Step 1: Select Destination & Service Package';
                stepContainer.innerHTML = `
          <div class="form-group">
            <label>Destination Body</label>
            <select id="wizDestination" class="form-control">
              ${CELESTIAL_BODIES.map((b) => `<option value="${b.id}" ${b.id === currentForm.destinationId ? 'selected' : ''}>${b.name}</option>`).join('')}
            </select>
          </div>
        ...
        `;
                const destSelect = document.getElementById('wizDestination');
                ...
                const svcSelect = document.getElementById('wizService');
            ...
            case 2:
                if (stepTitle)
                    stepTitle.textContent = 'Step 2: Launch Logistics & Spacecraft';
                stepContainer.innerHTML = `
          <div class="form-group">
            <label>Departure Spaceport Hub</label>
            <select id="wizSpaceport" class="form-control">
              ${SPACEPORTS.map((sp) => `<option value="${sp.id}" ${sp.id === currentForm.spaceportId ? 'selected' : ''}>${sp.name} (${sp.location})</option>`).join('')}
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select id="wizVehicle" class="form-control">
              ${LAUNCH_VEHICLES.map((v) => `<option value="${v.id}" ${v.id === currentForm.launchVehicleId ? 'selected' : ''}>${v.name} [Provider: ${v.provider}]</option>`).join('')}
            </select>
          </div>
        ...
        `;
                document.getElementById('wizSpaceport')?.addEventListener('change', (e) => {
                    this.manager.updateFormData({ spaceportId: e.target.value });
                });
                document.getElementById('wizVehicle')?.addEventListener('change', (e) => {
                    this.manager.updateFormData({ launchVehicleId: e.target.value });
                    this.renderWizardStep();
                });
            ...
            case 3:
                if (stepTitle)
                    stepTitle.textContent = 'Step 3: Stargate Schedule & Passenger Roster';
                stepContainer.innerHTML = `
          <div class="form-row">
            <div class="form-group">
        ...
        `;
                document.getElementById('wizDepDate')?.addEventListener('change', (e) => {
                    this.manager.updateFormData({ departureDate: e.target.value });
                });
                document.getElementById('wizRetDate')?.addEventListener('change', (e) => {
                    this.manager.updateFormData({ returnDate: e.target.value });
                });
                document.getElementById('wizPassCount')?.addEventListener('change', (e) => {
                    this.manager.updateFormData({ passengers: parseInt(e.target.value) || 1 });
                    this.renderWizardStep();
                });
                document.getElementById('wizName')?.addEventListener('input', (e) => {
                    this.manager.updateFormData({ passengerName: e.target.value });
                });
                document.getElementById('wizEmail')?.addEventListener('input', (e) => {
                    this.manager.updateFormData({ passengerEmail: e.target.value });
                });
                break;
            case 4:
                if (stepTitle)
                    stepTitle.textContent = 'Step 4: Mission Addons & Radiation Shielding';
        ...
        `;
                ...
                document.getElementById('wizNotes')?.addEventListener('input', (e) => {
                    this.manager.updateFormData({ notes: e.target.value });
                });
                break;
        }
    }
